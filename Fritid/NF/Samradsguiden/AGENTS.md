# Samrådsguiden — projektinstruktioner (Codex/Cursor)

Den här filen gäller för arbete i `Fritid/NF/Samradsguiden/` och läses automatiskt av Codex och av Cursor (som stödjer `AGENTS.md` i projektroten och undermappar). En identisk fil, `CLAUDE.md`, finns i samma mapp för Claude Code — håll de två i synk när någon av dem ändras.
...
- Codex, installerad: `C:\Users\kentl\.codex\skills\samradsguiden\`.
...
Kent committar och pushar själv via Cursor som huvudregel. Ett AI-verktyg gör det bara på uttrycklig begäran (t.ex. vid krångel). Read-only git-kommandon (status, diff, log, remote -v, fetch) är alltid okej.
