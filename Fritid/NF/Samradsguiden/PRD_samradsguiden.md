...
Delvis. Det är viktigt att verktyget inte överdriver skillnaden, eftersom den redan dokumenterade metoden (`Skills/samradsunderlag/SKILL.md`) visar att **grundstrukturen är gemensam**: Statsrådsberedningens anvisning gäller myndighetsövergripande, och Boverket — en renodlad PBL-myndighet — tillämpar uttryckligen samma ordning som i MB-exemplet (Boverket, 2026). Sexdelsmodellen (ärendets ram, sammanfattning, referat, synpunkter i underlagets ordning, vad som begärs, källförteckning) är alltså inte MB-specifik.
...
5b. **Löst 16 augusti 2026, samma dag:** Kent uppmärksammade att även Boverket-länken till "Kommunala exempel på frivillig kompensation" (`/verktyg/kompensation/exempel/`) gav 404, trots att den ursprungligen hade flaggats "via sökträff — direkthämtning gav tekniskt fel". Sökt fram korrekt nuvarande sida: Boverket har slagit ihop undersidan med föräldrasidan `/verktyg/kompensation/` (utan `/exempel/`), som nu innehåller både Göteborgs- och Lomma-exemplen i sin helhet, direktverifierad i webbläsaren. Länken och den fullständiga referensen är rättad i källförteckningen nedan och i `fordjupning.html`. Samma tillfälle användes till att kontrollera samtliga övriga externa länkar i källförteckningen (Ekoplankompassen, EU-kommissionen, Forest Trends, GOV.UK, IEEP, Miljösamverkan Sverige, Naturskyddsföreningen Norrköping, Naturvårdsverket 2021/2023, regeringen.se, riksdagen.se, US EPA) — samtliga bekräftat nåbara denna dag.
5c. **Löst 16 augusti 2026, samma dag:** Kent uppmärksammade att länken till `Skills/samradsunderlag/SKILL.md` i `NCC_stenbryttning`s teknik-modal gav 404 live, trots att filen finns och är pushad i repot. Grundorsak: `kentlundgren.github.io/Codex` byggs av GitHub Pages via Jekyll (inget `_config.yml`/`.nojekyll`), och Jekyll bygger om `.md`-filer som har YAML-frontmatter (vilket alla Claude Skill-filer har) till `.html` — originalfilens `.md`-sökväg slutar då 404:a, bara `.html`-varianten fungerar. Kent föreslog själv rätt lösning: länka till GitHub:s egen blob-vy (`github.com/kentlundgren/Codex/blob/main/...`) i stället, som alltid fungerar oavsett Jekyll-bygget. Ändrat i `NCC_stenbryttning/index.html` och `struktur.html` (SKILL.md-länken) samt, för konsekvens och framtida robusthet, i alla fyra länkar till `PRD_samradsguiden.md` i `Samradsguiden`s egna sidor (den filen saknar frontmatter och fungerade redan på sin `.md`-sökväg, men blob-länken är säkrare om frontmatter skulle läggas till senare). Sparat som teknisk projektkunskap i minnessystemet för att undvika samma fel i framtida `.md`-länkar i detta repo.
...
Boverket (2018) *Frivillig ekologisk kompensation i planering och byggande*, PBL kunskapsbanken, publicerad 13 juni, senast ändrad 15 juli 2026. Tillgänglig på: https://www.boverket.se/sv/PBL-kunskapsbanken/teman/ekosystemtjanster/verktyg/kompensation/ (Hämtad och innehåll direktverifierat 16 augusti 2026). *(Primärkälla för uppgifterna om Göteborgs stad och Lomma kommun som exempel på frivillig kompensation. Länken rättad 16 augusti 2026 efter att Kent uppmärksammade att den tidigare citerade undersidan `/exempel/` gav 404 — Boverket har sedan dess slagit ihop innehållet till denna sida, som verifierades direkt i webbläsaren, inte bara via sökträff som tidigare.)*

Boverket (2026) *Att svara på Boverkets remiss*. Tillgänglig på: https://www.boverket.se/sv/lag--ratt/boverkets-remisser/att-svara-pa-boverkets-remiss/ (Hämtad: 13 augusti 2026). *(Visar att Statsrådsberedningens sexdelsmodell tillämpas av en renodlad PBL-myndighet — grunden för avsnitt 5:s slutsats att dispositionsprincipen är gemensam över lagrummen.)*
